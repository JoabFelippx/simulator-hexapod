/*
troca de gait clicando em quadrado do Dabble
*/
...
// Avaliador genérico de Bézier 1D (De Casteljau) para até 6 pontos
static float bezier1D(const float* P, int count, float t){
  // count = número de pontos de controle (2..6)
  int n = count;
  float tmp[6];
  if(n < 2) return P[0];
  if(n > 6) n = 6; // segurança
  for(int i=0;i<n;i++) tmp[i] = P[i];
  float u = 1.0f - t;
  for(int k=1;k<n;k++){
    for(int i=0;i<n-k;i++){
      tmp[i] = tmp[i]*u + tmp[i+1]*t;
    }
  }
  return tmp[0];
}
...
struct Pata {
  ...
  // graus/quantidade de pontos para cada fase
  int supportBezierPoints; // 2 (linear), 3 (quadrática) ou 4 (cúbica)
  int swingBezierPoints;   // normalmente 4 (cúbica)
  ...
  // graus padrão das curvas
  this->supportBezierPoints = 2; // apoio linear por padrão
  this->swingBezierPoints = 4;   // swing cúbico por padrão
    this->P0[0] = 0.0;
    this->P0[1] = 0.0;
    this->P1[0] = 0.0;
    this->P1[1] = 0.0;
  ...
  floatxyz trajetoriaLinear(floatxyz xyz_ini, int k, int offset, float angle_rad){
    ...
    if (kn < METADE_PONTOS){
      ...
      float u = 1 - t;
      // Swing: manter cúbica (ou permitir no futuro swingBezierPoints)
      float Px0 = this->P0[0];
      float Px1 = this->P1[0];
      float Px2 = this->P2[0];
      float Px3 = this->P3[0];
      float Sx = u*u*u*Px0 + 3*u*u*t*Px1 + 3*u*t*t*Px2 + t*t*t*Px3;
      float Pz0 = this->P0[1];
      float Pz1 = this->P1[1];
      float Pz2 = this->P2[1];
      float Pz3 = this->P3[1];
      float Sz = u*u*u*Pz0 + 3*u*u*t*Pz1 + 3*u*t*t*Pz2 + t*t*t*Pz3;
      xyz.x = xyz_ini.x + c_ang*(-xyz_ini.x + Sx);
      xyz.y = xyz_ini.y + s_ang*(-xyz_ini.x + Sx);
      xyz.z = Sz;
    }
    else{
      // Segunda metade (apoio): Bézier dinâmica no plano (z constante)
      float t = float(kn - METADE_PONTOS) / (METADE_PONTOS - 1);
      float G0 = this->P3[0];
      float G3 = this->P0[0];
      float delta = G3 - G0;
      float ctrl[4];
      if (this->supportBezierPoints <= 2){
        ctrl[0] = G0;
        ctrl[1] = G3;
        float s = bezier1D(ctrl,2,t);
        xyz.x = xyz_ini.x + c_ang*(-xyz_ini.x + s);
        xyz.y = xyz_ini.y + s_ang*(-xyz_ini.x + s);
      } else if (this->supportBezierPoints == 3) {
        ctrl[0] = G0;
        ctrl[1] = G0 + delta * 0.5f;  // ponto médio suaviza aceleração
        ctrl[2] = G3;
        float s = bezier1D(ctrl,3,t);
        xyz.x = xyz_ini.x + c_ang*(-xyz_ini.x + s);
        xyz.y = xyz_ini.y + s_ang*(-xyz_ini.x + s);
      } else { // 4 pontos (cúbica)
        ctrl[0] = G0;
        ctrl[1] = G0 + delta * 0.33f;
        ctrl[2] = G0 + delta * 0.66f;
        ctrl[3] = G3;
        float s = bezier1D(ctrl,4,t);
        xyz.x = xyz_ini.x + c_ang*(-xyz_ini.x + s);
        xyz.y = xyz_ini.y + s_ang*(-xyz_ini.x + s);
      }
      xyz.z = xyz_ini.z; // mantém contato com o solo
    }
    return xyz;
  }
...
struct Hexapod {
  ...
  void configurarGait(int gait) {
    // Configura offsets e parâmetros de Bézier para cada gait
    switch(gait) {
      case GAIT_TRI: // Trigait - equilibrado, 3 patas por vez
        OFFSET_ESQF = 0;
        OFFSET_DIRT = METADE_PONTOS;
        OFFSET_ESQM = METADE_PONTOS;
        OFFSET_DIRF = METADE_PONTOS;
        OFFSET_ESQT = 0;
        OFFSET_DIRM = 0;
        // Apoio quadrático (suave), swing cúbico
        setAllPatasBezier(3, 4);
        Serial.println("GAIT: TRI (3+3)");
        break;

      case GAIT_WAVE: // Wave - mais lento, 1 pata por vez
        OFFSET_ESQF = 0;
        OFFSET_ESQM = QUARTO_PONTOS;
        OFFSET_ESQT = METADE_PONTOS;
        OFFSET_DIRF = METADE_PONTOS + QUARTO_PONTOS;
        OFFSET_DIRM = TOTAL_PONTOS - QUARTO_PONTOS;
        OFFSET_DIRT = QUARTO_PONTOS + QUARTO_PONTOS/2;
        // Apoio linear (mais contato), swing cúbico
        setAllPatasBezier(2, 4);
        Serial.println("GAIT: WAVE (1+1+1...)");
        break;

      case GAIT_RIPPLE: // Ripple - rápido, alternado
        OFFSET_ESQF = 0;
        OFFSET_ESQM = QUARTO_PONTOS + QUARTO_PONTOS/2;
        OFFSET_ESQT = QUARTO_PONTOS;
        OFFSET_DIRF = METADE_PONTOS;
        OFFSET_DIRM = METADE_PONTOS + QUARTO_PONTOS + QUARTO_PONTOS/2;
        OFFSET_DIRT = METADE_PONTOS + QUARTO_PONTOS;
        // Apoio cúbico (mais dinâmico), swing cúbico
        setAllPatasBezier(4, 4);
        Serial.println("GAIT: RIPPLE (2+2+2)");
        break;

      default:
        Serial.println("GAIT: PADRÃO (TRI)");
        configurarGait(GAIT_TRI);
        break;
    }
  }

  void setAllPatasBezier(int support, int swing) {
    EsqF.supportBezierPoints = support;
    EsqF.swingBezierPoints = swing;
    EsqM.supportBezierPoints = support;
    EsqM.swingBezierPoints = swing;
    EsqT.supportBezierPoints = support;
    EsqT.swingBezierPoints = swing;
    DirF.supportBezierPoints = support;
    DirF.swingBezierPoints = swing;
    DirM.supportBezierPoints = support;
    DirM.swingBezierPoints = swing;
    DirT.supportBezierPoints = support;
    DirT.swingBezierPoints = swing;
  }
...
void setup() {
  ...
  // Configuração inicial do gait
  scarlet.configurarGait(gaitAtual);
...
// Sistema de Gait (marchas)
enum TipoGait {
  GAIT_TRI = 0,    // Trigait - equilibrado
  GAIT_WAVE = 1,   // Wave - mais lento, estável
  GAIT_RIPPLE = 2, // Ripple - rápido
  GAIT_TOTAL = 3
};
int gaitAtual = GAIT_TRI;
...
void TaskComunicacao(void *pvParameters) {
  ...
  unsigned long lastGaitChangeMs = 0;
  const unsigned long debounceMs = 120; // debounce básico
  const unsigned long gaitDebounceMs = 500; // debounce para troca de gait (evita trocas rápidas demais)
  for (;;) {
    Dabble.processInput();
    
    // Trocar gait com botão Square (disponível quando armado)
    if (estado != 11 && GamePad.isSquarePressed() && (millis() - lastGaitChangeMs > gaitDebounceMs)) {
      gaitAtual = (gaitAtual + 1) % GAIT_TOTAL;
      scarlet.configurarGait(gaitAtual);
      lastGaitChangeMs = millis();
    }
