/*
Avaliador Bézier genérico 1D:
Adicionei uma função local bezier1D (De Casteljau) que avalia curvas com 2 a 6 pontos de controle, sem pow/binomial e com bom desempenho.
Parâmetros por pata:
Em Pata, criei:
supportBezierPoints: quantos pontos a fase de apoio usa (2=linear, 3=quadrática, 4=cúbica).
swingBezierPoints (já deixei um campo guardado para expansão futura, swing continua cúbica como antes).
Em setup(), ativei por padrão suporte quadrático para TODAS as patas:
EsqF/EsqM/EsqT/DirF/DirM/DirT.supportBezierPoints = 3
Fase Swing (primeira metade):
Continua com Bézier cúbica 3D (seu comportamento), com leve limpeza do código.
Fase Apoio (segunda metade):
Agora é Bézier dinâmica no plano (z constante), construída entre G0=P3[0] (toque) e G3=P0[0] (fim do apoio):
2 pontos: linear (reta, se quiser máxima simplicidade)
3 pontos: quadrática (default que ativei, já muito suave)
4 pontos: cúbica (super suave)
O x/y seguem a curva rotacionada por angle_rad; z permanece em contato com o solo.
*/
...
void setup() {
  ...
  // Configuração de curva de apoio dinâmica (Bézier quadrática) para todas as patas
  EsqF.supportBezierPoints = 3;
  EsqM.supportBezierPoints = 3;
  EsqT.supportBezierPoints = 3;
  DirF.supportBezierPoints = 3;
  DirM.supportBezierPoints = 3;
  DirT.supportBezierPoints = 3;
